...
LABELS100=tf.convert_to_tensor(LABELS100, dtype=tf.int64)
...
if __name__ == '__main__':
    ...
    PATH = '/share/lwz/simclr_model/r50_google/saved_model'
    pretrained_model = tf.saved_model.load(PATH)
    train_ds,info = tfds.load(name="imagenet2012", as_supervised=True, split="train",
                         data_dir='/share/datasets/tensorflow_datasets',with_info=True)
    print(info)
    # train_ds = train_ds.filter(lambda image, label: tf.reduce_any(tf.equal(label, LABELS100)))
    ...
    # test_ds = test_ds.filter(lambda image, label: tf.reduce_any(tf.equal(label, LABELS100)))
