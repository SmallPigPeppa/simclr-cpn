...
class LinearLayer(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    self._name = name
    if self.use_bn:
      self.bn_relu = resnet_semi.BatchNormRelu(relu=False, center=use_bias,dtype='float32')
  ...
  def build(self, input_shape):
    ...
    self.dense = tf.keras.layers.Dense(
        ...
        use_bias=self.use_bias and not self.use_bn,dtype='float32')
...
class Model(tf.keras.models.Model):
  ...
  def __init__(self, num_classes, **kwargs):
    ...
    self.resnet_model = resnet_semi.resnet(
  ...
  def __call__(self, inputs, training):
    ...
    projection_head_outputs, supervised_head_inputs = self._projection_head(
        hiddens, training)
    projection_head_outputs=tf.cast(projection_head_outputs,dtype='float32')
    supervised_head_inputs = tf.cast(supervised_head_inputs, dtype='float32')
